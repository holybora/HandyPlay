...
- `:navigation`
- `:feature:welcome`, `:feature:home`, `:feature:category`, `:feature:ttlcache`, `:feature:listvssequence`
...
- `ui/HandyPlayApp.kt` — Root composable: `HandyPlayTheme` → `Scaffold` → `NavHost` (start: `WelcomeDestination`, routes: `WelcomeDestination` → `HomeDestination` (pops Welcome), `HomeDestination` → `CategoryDestination`, `CategoryDestination` → `TtlCacheDestination` via `Topic.ID_TTL_CACHE`, `CategoryDestination` → `ListVsSequenceDestination` via `Topic.ID_LIST_VS_SEQUENCE`)
