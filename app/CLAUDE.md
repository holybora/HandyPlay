...
- `ui/HandyPlayNavHost.kt` — NavHost routes: `WelcomeDestination` → `HomeDestination` (pops Welcome), `HomeDestination` → `CategoryDestination`, `CategoryDestination` → `TtlCacheDestination` via `Topic.ID_TTL_CACHE`, `CategoryDestination` → `ListVsSequenceDestination` via `Topic.ID_LIST_VS_SEQUENCE`
